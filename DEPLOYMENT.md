...
multiplayer combat, multiplayer Outlaw, UI, ability art, and map rendering on
nine separate Ubuntu runners. Every existing test still runs; multiplayer scenarios remain sequential
within each runner because they bind fixed UDP ports. Each runner imports its
own assets, and only the three visual groups install Xvfb. A failed group does
