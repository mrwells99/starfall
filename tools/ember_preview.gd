...
func _ready() -> void:
 ...
 model = load("res://assets/characters/ember.glb").instantiate()
 add_child(model)
 model.rotation.y = PI
 ...
 var title:=Label.new();title.text="EMBER";title.add_theme_font_size_override("font_size",28);column.add_child(title)
 ...
 capture_mode="--ember-capture" in OS.get_cmdline_user_args()
...
func _process(_delta:float)->void:
 if capture_mode:
  ...
  if capture_frames==44:
   get_viewport().get_texture().get_image().save_png("res://artifacts/ember/godot-walk.png")
   get_tree().quit()
