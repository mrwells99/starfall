"""Fit a backed-up class costume to the verified v2 mannequin and motion.
Staged outputs only. Run Blender --python this_file -- --class CLASS.
"""
...
import model_forge_equipment as equipment
parser = argparse.ArgumentParser()
parser.add_argument('--class', dest='slug', choices=['ember','luminary','vanguard'], required=True)
args = parser.parse_args(sys.argv[sys.argv.index('--')+1:])
SLUG = args.slug
TITLE = SLUG.title()
SOURCE = ROOT / ('art_source/' + SLUG + '_ual')
LIBRARY = ROOT / 'art_source/model_forge_v2/AnimationLibrary_Godot_Standard.glb'
record_input = json.loads((SOURCE/'class_record.json').read_text(encoding='utf-8'))
assert hashlib.sha256(LIBRARY.read_bytes()).hexdigest() == record_input['inputs']['library_sha256'], 'Preset library changed'
assert hashlib.sha256((SOURCE/'costume_source.blend').read_bytes()).hexdigest() == record_input['inputs']['costume_sha256'], 'Costume input changed; record intentional revisions first'
OUT = ROOT / 'artifacts/forge-v2-all-classes' / SLUG
OUT.mkdir(parents=True, exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(SOURCE/'costume_source.blend'))
bpy.context.preferences.filepaths.save_version=0
old = bpy.data.objects[TITLE + '_Rig']
old.animation_data_clear()
for p in old.pose.bones: p.matrix_basis = Matrix.Identity(4)
body = bpy.data.objects[TITLE + '_SkinnedModel']
for obj in list(bpy.data.objects):
    if obj not in [old, body]: bpy.data.objects.remove(obj, do_unlink=True)
...
rig.name = TITLE + '_UAL_Rig'
mannequin = bpy.data.objects['Mannequin']
for obj in list(bpy.data.objects):
    if obj not in [old, body, rig, mannequin]: bpy.data.objects.remove(obj, do_unlink=True)
...
equipment.prepare(SLUG, rig, old_bones, transforms)

# Class-specific cloth and equipment controls extend the library skeleton. Its
...
for name in extras:
    ...
    parent = src['parent']
    b.parent = rig.data.edit_bones['root' if name in ['staff','weapon'] else mapping.get(parent,parent)]
...
# Remove covered head geometry; keep each class's sealed hood or helmet.
...
mannequin.name=TITLE + '_UAL_Undersuit'
mannequin.data.materials.clear(); mannequin.data.materials.append(bpy.data.materials[TITLE + ('_JointLeather' if SLUG=='vanguard' else '_BlackLeather')])
...
if SLUG=='vanguard': plans['Strike']=('Sword_Attack',0,False)
samples={}
...
loop_names = [n for n in plans if n not in ['CastEnter','CastRelease','CastExit','JumpStart','JumpLand','Strike']]
    ...
    for i,pose in enumerate(frames):
        ...
        for b in all_bones:
            ...
            if b.name in core:
                ...
                if b.name.startswith(('robe','hem','mantle','cape','tabard')):
        ...
        bpy.context.view_layer.update()
        equipment.pose(SLUG, rig, label, t)
        for b in all_bones:
            p=rig.pose.bones[b.name]
...
for screen in bpy.data.screens:
    for area in screen.areas:
        if area.type=='VIEW_3D':
            ...
            space.shading.type='MATERIAL'; space.shading.color_type='MATERIAL'
...
bpy.ops.object.select_all(action='DESELECT')
for o in [rig,body,mannequin]: o.select_set(True)
...
record={'library_sha256':hashlib.sha256(LIBRARY.read_bytes()).hexdigest(),
        ...
        'class':SLUG,'equipment':equipment.record(),'bone_map':mapping,'animations':{n:{'source':v[0],'heading_degrees':v[1],'upper_body_heading_degrees':upper_body_heading(v[1],v[2]),'pelvis_heading_degrees':v[1]*.65,'reversed':v[2],'seconds':samples[n][0],
            'loop':n in loop_names} for n,v in plans.items()}}
(SOURCE/'recipe.json').write_text(json.dumps(record,indent=2),encoding='utf-8')
print('MODEL_FORGE_CLASS_BUILD_COMPLETE', SLUG,len(all_bones),'bones',len(plans),'clips')
