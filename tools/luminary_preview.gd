...
func _ready() -> void:
 ...
 model = load("res://assets/characters/luminary.glb").instantiate()
 add_child(model)
 model.rotation.y = PI
 ...
 env.environment.ambient_light_color=Color("8b9cc0")
 env.environment.ambient_light_energy=.35
 env.environment.tonemap_mode=Environment.TONE_MAPPER_FILMIC
 add_child(env)
 add_light(Vector3(-2,4,-3),Color("ffe0bf"),.40)
 add_light(Vector3(3,2,-1),Color("abc9ff"),.20)
 add_light(Vector3(0,3,3),Color("b6c9ff"),.30)
 ...
 var title:=Label.new();title.text="LUMINARY";title.add_theme_font_size_override("font_size",28);column.add_child(title)
 ...
 capture_mode="--luminary-capture" in OS.get_cmdline_user_args()
...
func _process(_delta:float)->void:
 if capture_mode:
  ...
  if capture_frames==44:
   get_viewport().get_texture().get_image().save_png("res://artifacts/luminary/godot-walk.png")
   get_tree().quit()
