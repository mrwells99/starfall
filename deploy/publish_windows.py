#!/usr/bin/env python3
"""Verify, stage, or promote a Windows release. Run promote AFTER server health checks."""
import argparse
import hashlib
import json
...
def verify(folder, tag):
    if not re.fullmatch(r"sha-[a-f0-9]{7}", tag):
        raise ValueError("A paired Windows release requires an immutable sha-xxxxxxx tag")
    ...
    if (manifest.get("schema") != 1 or not re.fullmatch(r"[a-f0-9]{40}", build)
            ...
            or manifest.get("url") != f"https://play.leafmods.com/downloads/releases/{tag}/Starfall-Windows.zip"):
        raise ValueError("Windows client does not match the server release")
    archive = folder / "Starfall-Windows.zip"
    if not 0 < archive.stat().st_size <= 1 << 30 or archive.stat().st_size != manifest["size"] or digest(archive) != manifest["sha256"]:
        raise ValueError("Windows archive failed verification")
    files = manifest["files"]
    if not {"Starfall.exe", "Starfall.pck"}.issubset(files) or not 2 <= len(files) <= 128:
    ...
    with zipfile.ZipFile(archive) as z:
        ...
        for entry in z.infolist():
            ...
            if (name not in files or name.lower() in seen
                    ...
                    or (name not in ("Starfall.exe", "Starfall.pck") and not name.lower().endswith(".dll"))):
    ...
    launcher = folder / "StarfallLauncher.exe"
    expected_launcher = (folder / "StarfallLauncher.exe.sha256").read_text().split()[0]
...
def stage(source, root, tag):
    manifest = verify(source, tag)
    ...
    if destination.exists():
        ...
        existing = verify(destination, tag)
    ...
    try:
        for name in ("manifest.json", "Starfall-Windows.zip", "StarfallLauncher.exe", "StarfallLauncher.exe.sha256"):
...
def promote(root, tag):
    source = root / "releases" / tag
    verify(source, tag)
    for name in ("StarfallLauncher.exe", "StarfallLauncher.exe.sha256"):
...
if __name__ == "__main__":
    ...
    if args.action == "stage":
        ...
        stage(args.source, args.root, args.tag)
    elif args.action == "verify":
        verify(args.root / "releases" / args.tag, args.tag)
    else:
        promote(args.root, args.tag)
    print(f"Windows release {args.tag}: {args.action} complete")
