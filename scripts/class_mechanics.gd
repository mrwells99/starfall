...
static func validate(game, a, spell: Dictionary, b) -> String:
	...
	if spell.kind in ["inward", "outward", "orbit", "collapse"]:
		...
		if a.position.distance_to(s.anchor_pos) > 28 or not point_los(game, a.position, s.anchor_pos):
			return "Anchor out of range or sight"
		if spell.kind in ["inward", "outward"] and (b.position.distance_to(s.anchor_pos) > 28 or not point_los(game, s.anchor_pos, b.position)):
			return "Target out of anchor range or sight"
	return ""

static func enemies(game, a, center: Vector3, radius: float) -> Array:
	var out: Array = []
	for b in game.actors.values():
		if b.hp > 0 and b.team != a.team and game.may_harm(a, b) and center.distance_to(b.position) <= radius and point_los(game, center, b.position):
			out.append(b)
	return out
...
static func heal(game, a, b, amount: float) -> void:
	if b.hp <= 0 or not can_help(game, a, b):
		return
	amount = minf(100 - b.hp, amount * (1 - clampf((game.elapsed - 60) / 180.0, 0, 0.7)))
	b.hp += amount
	...
	match spell.kind:
		...
		"collapse":
			for other in enemies(game, a, s.anchor_pos, 6):
				...
				control(game, a, other, 2, "Collapse", true)
			s.anchor_left = 0.0
			s.orbit = 0.0
		_:
			return false
...
static func tick(game, a, delta: float) -> void:
	...
	if s.anchor_left > 0 and s.orbit > 0:
		for b in enemies(game, a, s.anchor_pos, 6):
