...
func apply(vertical_speed: float, airborne: bool, delta: float, casting: bool) -> void:
 ...
 var old_hand := skeleton.get_bone_global_pose(hand).origin
 for i in bones.size():
 ...
 # The orb is baked on an independent control. Translate its parent focus
 # by the hand's correction while retaining the authored orbit rotations.
 var correction := skeleton.get_bone_global_pose(hand).origin - old_hand
