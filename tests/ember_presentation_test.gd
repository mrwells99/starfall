...
func run() -> void:
 ...
 actor.setup(1,1,0,"Ember")
 var visual = actor.champion_model
 var art = visual.ember_art
 check(art != null, "Ember uses the authored presentation")
 ...
 print("Ember presentation: %d/%d checks passed" % [checks-failures,checks])
