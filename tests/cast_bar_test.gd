...
func run() -> void:
	...
	for champion in game.Kits.NAMES:
		...
		for slot in actor.kit.size():
			...
			var immune: bool=champion=="Outlaw" and spell.kind in ["severe","defense_detonation","deadeye"]
	...
	actor.champion="Outlaw";actor.kit=game.Kits.get_kit("Outlaw");actor.casting=0;actor.cast_left=.3
