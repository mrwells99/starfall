...
func run() -> void:
 ...
 for index in clips.size():
  ...
  var source_offset: Vector3 = rig.get_bone_global_pose(blend.focus).origin-rig.get_bone_global_pose(blend.hand).origin
  ...
  var target_offset: Vector3 = rig.get_bone_global_pose(blend.focus).origin-rig.get_bone_global_pose(blend.hand).origin
  blend.apply(0)
  ...
  var offset: Vector3 = rig.get_bone_global_pose(blend.focus).origin-rig.get_bone_global_pose(blend.hand).origin
  check(offset.distance_to((source_offset+target_offset)*.5)<.0001,"Weapon attachment interpolates with the blended hand: "+target)
  restore(rig,after); blend.apply(seconds*.5)
 ...
 print("Fulcrum pose transitions: %d passed / %d total" % [checks-failures,checks])
