"""Exercise Ember skating, Ash concealment, early recall and reformation over real ENet."""
import os
...
base = [os.environ.get("GODOT", "godot"), "--headless", "--path", str(root),
        "--script", "tests/ember_particle_network_peer.gd", "--", *sys.argv[1:]]
...
def read_host():
    for line in host.stdout:
        lines.append(line)
        if "EMBER PARTICLE NETWORK READY" in line:
            ready.set()
...
try:
    if not ready.wait(30):
        raise RuntimeError("Ember test host did not start")
