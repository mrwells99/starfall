...
func run() -> void:
	...
	ck(a.cooldowns[1]==9 and a.cooldowns[2]==9 and a.cooldowns[3]==9 and a.cooldowns[7]==0,"Range variants share only their polarity cooldown")
	...
	ck(b.identity.entropy_dots.is_empty() and is_equal_approx(health-b.hp,150),"Cleanse backlash deals 10% max health")
	ck(game.CC.remaining(b,"silence")==3,"Cleanser is silenced for three seconds")
	Rules.cleanse_entropy(game,b,b);ck(is_equal_approx(health-b.hp,150),"An empty cleanse cannot retrigger backlash")
	...
	step(.3);ck(b.hp==1320,"Ten-meter right slash damages once across its sweep")
	...
	step(.3);ck(b.hp==1140,"Left deals one separate hit")
	...
	ck(b.hp==870 and a.identity.divide_ready==0 and a.identity.meditation==34,"Divide deals one hit, consumes unlock and costs no extra resource")
	step(.3);ck(b.hp==870,"Lingering Divide does not hit the same target twice")
	...
	step(.31);ck(b.hp==0,"Flow boosts Divide by 50% below 30% maximum health")
	...
	ck(b.hp==1230,"An enemy entering the lingering Divide is hit once")
	step(.4);ck(b.hp==1230,"Later rift ticks only slow")
	ck(Rules.in_ruin(Vector3(9,0,-3),Vector3.ZERO,0,0,1,false),"Ruin reaches inside the broad 150-degree cone")
	ck(not Rules.in_ruin(Vector3(0,0,3),Vector3.ZERO,0,0,1,false),"Ruin does not strike behind its cone")
