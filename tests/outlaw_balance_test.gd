...
func balance() -> void:
	...
	ck(is_equal_approx(-a.velocity.z,game.MovementTuning.FORWARD_SPEED*1.25),"Roll haste increases running speed by exactly 25 percent")
	a.walking=true; tick(a,.1)
	ck(is_equal_approx(-a.velocity.z,game.MovementTuning.FORWARD_SPEED*.5*1.25),"Roll haste increases voluntary walking speed by 25 percent")
	game.Outlaw.tick(game,a,5)
	ck(a.identity.roll_haste==0,"Roll haste expires after its five-second window")
	await reset()
	a.identity.instant_severe=1.5; b.hp = 1095
	ck(game.try_spell(1,1,2) and b.hp == 945,"Severe rounds in authored units before applying damage scaling")
	...
	ck(is_equal_approx(Vector2(b.velocity.x,b.velocity.z).length(),game.MovementTuning.FORWARD_SPEED*.4),"Severe reduces running speed by exactly 60 percent without double-applying walking")
	b.identity.slow=2; tick(b,.1)
	ck(is_equal_approx(Vector2(b.velocity.x,b.velocity.z).length(),game.MovementTuning.FORWARD_SPEED*.4),"The strongest slow applies instead of multiplying two slows")
	b.identity.immune=1; tick(b,.1)
	ck(is_equal_approx(Vector2(b.velocity.x,b.velocity.z).length(),game.MovementTuning.FORWARD_SPEED),"Root/slow immunity suppresses Severe's slow")
	...
	game.damage(b,a,20)
	ck(a.hp==1400,"Airborne Backflip reduces incoming damage by 50 percent from launch")
	a.shield=1; game.damage(b,a,20)
	ck(a.hp==1320,"A stronger shield remains stronger without multiplying defenses")
	a.shield=0; a.identity.backflip_active=false; game.damage(b,a,20)
	ck(a.hp==1120,"Backflip damage reduction ends with Backflip")
	await reset()
...
func pillars() -> void:
	...
	for center in game.Layout.cover_centers():
		for normal in [Vector3.LEFT,Vector3.RIGHT,Vector3.FORWARD,Vector3.BACK]:
			...
			var start: Vector3=a.position
			var speed: float = game.MovementTuning.BACKWARD_SPEED if direction.z>0 else game.MovementTuning.FORWARD_SPEED
