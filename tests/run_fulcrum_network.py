"""Exercise aimed Fulcrum abilities and charge replication over real ENet."""
import os
...
base = [os.environ.get("GODOT", "godot"), "--headless", "--path", str(root),
        "--script", "tests/fulcrum_network_peer.gd", "--", *sys.argv[1:]]
...
def read_host():
    for line in host.stdout:
        lines.append(line)
        if "FULCRUM NETWORK READY" in line:
            ready.set()
...
try:
    if not ready.wait(30):
        raise RuntimeError("Fulcrum test host did not start")
