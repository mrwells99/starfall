...
The arena is an original floating ruin with a procedural base and one authored quality section. The material/atmosphere pass lives in `arena_world.gd`, `arena_atmosphere.gd`, `arena_sky.gd` and their shaders. The owner subsequently approved Blender-authored architecture and baked lighting. The new section replaces the cover at (-6,0,5), its 12 x 14 m floor patch, and approximately 12 m of west perimeter. Its 13 mesh batches have real bevels, arched relief, fitted and split flagstones, bronze ornament, UV-based normal maps and native LightmapGI lighting. Other cover remains procedural. Editable sources are ignored under `assets-source/`; only compressed meshes ship. See `ART_SLICE_HANDOFF.md` for the complete rebuild workflow.
...
**No textures need to be generated by the user.** The authored section supplies nine original 512 x 512 PNGs: albedo, normal and roughness for basalt, floor and bronze. All tile over four metres, have exact matching opposite-edge pixels, and keep lighting out of albedo. These total 3,273,616 bytes. The existing 512 x 512 slate remains for procedural geometry. Native baked lighting uses a separate HDR EXR atlas; it is an atlas, not a tiling surface texture. The complete runtime environment folder, including compressed meshes and lighting, is approximately 7.8 MiB. Editable Blender/GLB sources are ignored outside that folder. The cached sky panorama is runtime memory, not a shipped image asset.
