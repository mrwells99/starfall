...
**Animation selection default (owner instruction, 2026-09-12):** show all relevant library candidates on a neutral, identity-free mannequin before applying a new animation to a character. Wait for the owner's choice; preserve rebuilt clips in the reusable local cache. See [ANIMATION_WORKFLOW.md](ANIMATION_WORKFLOW.md) for the complete approval, preview and caching rules.
...
**Current authorization:** The owner requested ability art and player models on 2026-09-07, superseding the earlier blanket art deferral for these areas. The owner subsequently requested the Cosmic Sanctum environment and its material/atmosphere upgrade. Audio remains separate work. The owner authorized the Blender skeletal pipeline for Ember, Luminary and Fulcrum on 2026-09-08; see below.
...
- **Vanguard:** Latest supplied reference is a heavy crystal-armored knight. The live Blender model has a sealed T-visor crystal-crown helmet, layered forged plate, violet resolve core, navy embroidered tabard/cape and a metal-frame hammer with inset crystal windows. Eight previous clips remain, with added cape motion. Current implementation and visual limits: `VANGUARD_REFERENCE_REBUILD.md`.
...
- **Fulcrum:** Blender-authored gravity heretic with a sealed obsidian mask, pointed dark hood, jagged armor, torn mantle, violet stoles and suspended orbit relics. No human face or hair geometry is present; see below.
- **All four champions use imported skinned Blender models**, dispatched by `scripts/champion_model.gd` to their dedicated presentation modules (`ember_art.gd`, `vanguard_authored.gd`, `luminary_art.gd`, `fulcrum_art.gd`). The generic fallback retains procedural geometry for unknown entries. Vanguard includes a weapon bone and confirmed-hit hammer animation; gameplay and collision remain on the parent actor.
...
## Fulcrum — masked Blender model and shared locomotion (2026-09-08)

The owner requested the same local Blender workflow and mostly unchanged animations using the gravity-heretic reference, preserved at `art_source/references/fulcrum.png`. The design has a pointed dark hood over a sealed angular obsidian mask, narrow violet fissures, overlapping black chest and limb armor, jagged shoulder blades, a split long coat, violet stoles and three torn mantle tails. Dark orbit relics with broken violet arcs sit on the belt and back. There is no human face, scalp or hair, and the approved weapon version adds a black singularity orb, violet event horizon, broken bronze orbital rails and suspended debris above an upward-cupped left hand.

Source: `art_source/fulcrum.blend`; runtime: `assets/characters/fulcrum.glb`; viewer: `scenes/fulcrum_preview.tscn`. The source contains 83,958 vertices, 157,160 triangles, 17 surfaces across two meshes, 12 unique materials and 62 bones. Five original packed 1024px textures provide violet weave, dark cloth color, fabric normal detail, rubbed steel color and variable steel roughness. The hood's trim follows its corrected outer surface; the closed mask shell extends into the hood. Mantle tears have irregular modeled contours rather than rectangular grid holes.

Idle, Walk, Run, WalkBackward, StrafeLeft, StrafeRight and Cast preserve Ember's core gait and durations. Six added mantle controls provide delayed cloth motion and four weapon controls supply hovering/counterrotation; the left arm carries the orb while other body channels preserve their established motion; the coat and stoles use the shared robe controls. The saved source plays Walk at 30 fps, frames 1–37. `tools/build_fulcrum.py` requires `tools/fulcrum_details.py` and the final weapon stage `tools/fulcrum_weapon.py`; `tools/verify_fulcrum.py` checks normalized weights, stance targets, loop closure, mantle motion and gait parity against the Ember source. `scripts/fulcrum_art.gd` follows current Ember responsiveness: measured speed selects locomotion directly, transitions into locomotion/idle have zero crossfade, and entering Cast uses 0.12 seconds. Smoothed cadence, team color and actor-state presentation remain intact; the authored source clips are unchanged by this runtime choice.

Final verification: normalized weights on all 83,958 vertices, closing poses for all seven clips, stance ankle-height error below 2.83e-7 m, six moving mantle controls, and less than 1.79e-7 difference across 392 core-gait matrices against Ember. Godot passed Fulcrum 1,068/1,068, combat 81/81 and rendered ability/art 179/179 on the final runtime module. Added one-frame movement and imported-pose equality checks verify immediate locomotion transitions. The final front/back renders, four walk/run samples and Godot capture were inspected, and the editable source was opened in Blender.

The result remains a stylized interpretation with simpler surface detail than the concept; the owner explicitly approved the appearance and weapon integration. The approved body geometry, material settings and texture images are preserved; a pre-weapon backup is available at `artifacts/fulcrum_before_weapon/`. Cloth uses authored bones without live collision simulation, there is no terrain foot IK or dedicated jump/death clip, and fast backward/sideways travel can retain the inherited foot-sliding limitation. No new spell VFX or gameplay changes are included. Rebuild commands, complete files and validation details are in `CHARACTER_PIPELINE.md`.
