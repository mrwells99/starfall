...
### Jump momentum, shorter targeting ranges and Fulcrum revision (0.10.0)

`arena.gd::simulate_movement` computes horizontal input velocity on the ground and preserves the resulting world-space velocity while airborne. Releasing or reversing movement, turning, and expiration of speed modifiers during the jump do not erase takeoff momentum. Collisions remain physical; stun, root and Hold the Line stop horizontal movement. Death clears velocity. `combatant.gd` adds authoritative `grounded` to snapshots. After reconciliation, `movement_prediction.gd` uses that grounded value for the first replayed step, then returns to the local collision result; this avoids using stale floor contact after rewinding position.

`kits.gd::spell` scales positive targeting ranges by 0.75, rounds to the nearest 0.5m and caps at `MAX_CAST_RANGE = 18`. Opposing arena spawn points are 20m apart, so hostile spells cannot immediately target the opposing spawn. Target-centred area radii and self-mobility distances retain their own authored rules. Anchor and ongoing support-link validation also use the 18m cap. Bot approach distances follow their current kit ranges. Both cast start and resolution use authoritative range validation.

Victims retain two separate source-keyed DoT dictionaries in replicated identity. `dots` is Graviton: six initial damage per cast, eleven-second duration, three damage per one-second tick per stack, at most two stacks from each caster, no Meditation. A further application refreshes both stacks. `entropy_dots` holds Entropy: no initial damage, fifteen seconds, two damage plus five Meditation per tick, one refreshing stack per caster. Refreshing either effect preserves its next tick time. `tick_dots` limits ticking to the effect's remaining lifetime even when a simulation step crosses expiry, caps Meditation at 100, and removes effects on source loss, death or loss of world combat permission. Resource credit occurs before damage so duel-end identity resets win. Completed DPS Mend clears both families; hazards remain independent.

Successful Inward grants `instant_collapse`, a single stored charge without an expiry timer. A charged Collapse bypasses cast time, movement casting restrictions and the global cooldown; it neither starts nor resets an existing GCD. It retains its individual 18s cooldown, anchor/range restrictions and normal control/lockout validation. Failed attempts preserve the charge; successful resolution consumes it. The hotbar flashes both this charge and the existing instant-Graviton charge and omits the GCD overlay from charged Collapse. Collapse still samples 75 Meditation for its stun without spending Meditation, and a hit still grants instant Graviton.

Fulcrum Starfall keeps its 50-Meditation requirement and two-second cast. On valid completion it spends all Meditation once and deals `20 + 0.4 * Meditation` to each legal enemy within `STARFALL_RADIUS = 5` metres of the selected target. Splash requires line of sight from the target centre and respects world duel permissions. `StarfallWarning` follows the selected target during the cast and uses the same radius. Luminary's separate Starfall keeps its damage/healing identity.

`KIT_SIZE = 14`; Entropy defaults to Shift+7, after Fulcrum Starfall on Shift+6. Other class kits remain twelve. Existing hotbar assignments and both binding columns are preserved when filling an unused slot; if Shift+7 conflicts with any saved action or ability binding, the new slot is left unbound and clickable. Entropy's generated icon is installed in `assets/icons/abilities/entropy.png`, with its original image and prompt in `art_source/icons/entropy/`. `session_social.gd` positions chat 18px from the bottom-left with 160px height; the old tutorial label is removed. Enter/Escape and existing chat validation remain intact.

Regression coverage: `jump_momentum_test.gd`, `movement_bindings_test.gd`, `fulcrum_meditation_test.gd`, `spell_range_test.gd`, `layout_migration_test.gd`, `social_test.gd`, plus the existing combat, class identity, rendered UI/art and real ENet movement/latency tests. Protocol version 0.10.0 requires matching restarted clients and servers; no deployment is part of this change. The character models and frozen Starfall Character Forge v1 archive are not rebuilt or restored.

Validated on the installed Godot 4.7.2 (September 8, 2026): Fulcrum 111/111, jump momentum 35/35, movement bindings 23/23, range 73/73, layout migration 15/15, social 26/26, class identity 91/91, combat 81/81, rendered art 183/183 and UI 198/198. Real ENet host/client and the 150ms movement variant passed, including authoritative airborne momentum, client release/reverse, landing stop, actual damage/resource/cooldown snapshots, rematch and disconnect takeover. Version, RPC schema and legacy-server rejection tests passed. The network harness now waits for actual host readiness, enters the shortened combat range after movement assertions, and supports the installed Windows executable via `GODOT`. Review image: `artifacts/general-gameplay-review.png`; logs: `artifacts/fulcrum-final-check.log`, `artifacts/general-network-latency-movement.log`, `artifacts/general-handshake.log`. Rendered checks used Compatibility on the RTX 5060 Ti; the project's Forward+ High default was preserved.
