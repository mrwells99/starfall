...
### High map-image stability comparison

Graphics-preset selection applies immediately and writes the chosen preset to local `user://starfall.cfg`. Startup restores it; no account/server setting is involved. Other pending settings still require Apply. Programmatic menu initialization does not trigger selection/save events.

Owner-approved High-only treatment, optimized after its initial comparison: 4× MSAA, up to 1.10× spatial supersampling per axis at native resolution (21% additional pixels, reduced from 56.25%), capped at a 3840×2160 pixel budget for extra sampling. Existing lower-resolution selections are respected; Balanced/Performance keep their prior sampling. `map_image_quality.gd` enables derivative-based stone highlight filtering, averages unresolved procedural grit, softens map-only normal glints by 20%, and retains authored arena detail farther away. No source textures, geometry, collision, animation or gameplay are replaced. Material/LOD values restore exactly on switching away. TAA was compared locally but not retained because the image was noticeably softer; no temporal history/trails are introduced by this treatment. Native HUD rendering stays unchanged. High retains SSAO, volumetric atmosphere, glow and shrine shadows but no longer layers SSIL over baked map lighting. This can slightly change indirect-light response; less oversampling can leave more subpixel aliasing than the first expensive High. This targets aliasing, not monitor tearing or uneven CPU frame timing.

Godot 4.7.2 checks after High optimization: map image-quality switching 61/61, resolution 12/12, preset persistence 15/15. Earlier unchanged map/navigation checks passed 57/57. Local rendered stairs/inlay/rift comparisons and probe notes remain in ignored `local_resources/diagnostics/`. The completed camera diagnostic hooks were removed; its three capture records remain local.

Short repeated panning comparisons at 1920×1078 on RTX 5060 Ti reduced map-only GPU time by 24–33% against the first High; this excludes combat/CPU costs and is not a guaranteed FPS gain. Component isolation found reducing supersampling and disabling SSIL worthwhile; dropping fog, shadows or retained geometry detail did not justify the visual tradeoff in that sample.
...
`movement_controls.gd` clamps ordinary mouse pitch to ±89.1°, preserving yaw near the poles. `arena.build_camera` gives SpringArm3D a 0.20m sphere, 0.04m margin and mask 5 (terrain plus camera-only layer 3), with Camera3D near 0.05. The spring retracts for floor/wall/terrace/platform collision even below the 3m wheel minimum; the requested 3–9m zoom remains unchanged. The 40 decorative perimeter rocks opt into convex camera hulls built from their generated vertices, preventing the spawn/orbit camera from entering them above the boundary walls. These hulls are client-art-only; player collision and ability LOS still use mask 1, and dedicated servers do not build them. Small rubble remains collision-free. In open space the camera can orbit beneath the character. Existing interpolated target following and Outlaw shoulder-camera save/restore remain intact.

`camera_character_fade.gd` selects the followed gameplay actor from `arena._process`, restores on menu-camera switching or `clear_actors`, and applies per-pixel distance dithering to authored costume roots (including weapons and bone attachments). Surface distance 0.8m is invisible; 2.4m is opaque. Actor-owned presenter material instances retain their animation/flash references; unknown/shared materials receive temporary local overrides with exact restoration. Health/nameplate/ability-marker siblings are excluded. Dither works in Compatibility and Forward+ without forcing transparent blending; see [Godot distance fading](https://docs.godotengine.org/en/stable/tutorials/3d/standard_material_3d.html#proximity-and-distance-fade). Rendering-only state adds no network fields or gameplay changes.

Focused regression: `tests/camera_orbit_test.gd`; existing `camera_zoom_test.gd`, `movement_controls_test.gd` and `camera_interpolation_test.gd` cover zoom, input ownership, aiming and render-follow cadence. Native close/up/vertical captures were inspected in both Compatibility and Forward+ (`artifacts/camera-orbit/`), including floor clearance and equipment fading. Validation: 54 orbit/fade checks across all five champions, 33 zoom checks, 69 movement-control checks, and the camera interpolation probe passed.


## World Starwalk

`arena.gd`'s `world_mode` setter calls `arena_world.gd.set_world_starwalk` to build/remove the isolated `world_starwalk.gd` extension on both clients and dedicated hosts. Static box collision is generated from the same platform position/size functions everywhere; headless hosts skip all art. Existing arena walls and geometry remain unchanged. The entrance sits on the eastern terrace stair landing at (15, 1.2, 4.5) and requires an eight-second per-player authoritative dwell timer, with once-per-second private countdown feedback. Exit, death, duel entry, actor replacement and departure discard pending progress. It then teleports to an exterior starting island, avoiding alterations to the shared arena perimeter. The 72-jump spiral rises 59.04m with repeated 0.65–0.95m steps, below the unchanged normal jump apex. Narrow collision surfaces match their emissive outlines. The final island is 3.6m wide so its raised front edge leaves enough clearance for the last jump. `world_starwalk_scenery.gd` adds 28 batched meshes of World-only noncolliding cosmic scenery and freezes orbital motion when reduced effects is enabled; all scenery disappears with the course.

The existing authoritative `tick_world` handles gate entry, stage messages, completion and rescue below y=-6. Teleports clear jump/forced-travel state and increment the existing motion revision so prediction and remote interpolation treat them as displacements; no RPC or snapshot fields were added. Health and cooldowns remain intact except existing interrupted-channel refunds. The course test physically jumps the capsule between all 72 platform pairs at full keyboard speed, using standing takeoff positions that keep the full capsule plus 0.10m on the source and checking the destination collider, verifies rescue and finish behavior, and checks that mode changes remove colliders. Run `godot --headless --path . --script tests/world_starwalk_test.gd`; existing World and network World runners cover session regression. Local screenshot helper: `artifacts/starwalk/preview.gd` (ignored artifact).
