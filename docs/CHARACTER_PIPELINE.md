...
**Current Fulcrum exception (2026-09-09):** [Universal Animation pass r001](FULCRUM_UNIVERSAL_ANIMATION_PASS.md) supersedes the historical Fulcrum anatomy and gait below. At the owner's request, the existing costume now fits the actual downloaded Quaternius preset with its anatomy preserved and library-based locomotion/casting/jumps. This candidate is installed for feedback, with reversible originals; it is not yet the standard for other classes. Keep the frozen v1 archive intact.

**Named reusable procedure:** [`STARFALL_CHARACTER_FORGE_V1.md`](STARFALL_CHARACTER_FORGE_V1.md), “Starfall Character Forge v1.” The owner requested this preserved workflow for future classes. It includes the fixed process, current differences between classes, a verified archive of exact sources/assets, machine-readable animation contracts and the `$starfall-character-forge` skill. Use it as the repeatability entry point; this file retains per-class history.
